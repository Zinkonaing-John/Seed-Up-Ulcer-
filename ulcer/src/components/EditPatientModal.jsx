...
function EditPatientModal({ patient, isOpen, onClose, onSave }) {
  ...
  useEffect(() => {
    if (patient) {
      setFormData({
        ...
        mobility: patient.mobility || 4,
        nutrition: patient.nutrition || 4,
        friction_shear: patient.friction_shear || 3,
  ...
  const getRiskLabel = (score) => {
    if (score <= 9) return { label: 'Very High Risk', labelKr: '매우 높은 위험', color: 'text-red-600 bg-red-100' };
    if (score <= 12) return { label: 'High Risk', labelKr: '높은 위험', color: 'text-orange-600 bg-orange-100' };
    if (score <= 14) return { label: 'Moderate Risk', labelKr: '중간 위험', color: 'text-amber-600 bg-amber-100' };
    if (score <= 18) return { label: 'Mild Risk', labelKr: '낮은 위험', color: 'text-emerald-600 bg-emerald-100' };
    return { label: 'No Risk', labelKr: '위험 없음', color: 'text-emerald-600 bg-emerald-100' };
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto bg-white rounded-2xl p-6 shadow-2xl animate-slide-up">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">환자 정보 수정 - {patient?.name}</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Basic Info */}
          <div>
            <h3 className="text-sm font-semibold text-clinical-700 uppercase tracking-wider mb-3">Basic Information / 기본 정보</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="col-span-2">
                ...
                <input
                  ...
                  className={`w-full px-4 py-2.5 bg-slate-50 border rounded-xl text-slate-800 focus:outline-none focus:ring-2 transition-all ${
                    errors.name ? 'border-red-300 focus:border-red-500 focus:ring-red-100' : 'border-slate-200 focus:border-clinical-500 focus:ring-clinical-100'
                  }`}
                />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`w-full px-4 py-2.5 bg-slate-50 border rounded-xl text-slate-800 focus:outline-none focus:ring-2 transition-all ${
                    errors.age ? 'border-red-300' : 'border-slate-200 focus:border-clinical-500 focus:ring-clinical-100'
                  }`}
                />
              </div>
              <div>
              ...
              <div className="col-span-2 md:col-span-1">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`w-full px-4 py-2.5 bg-slate-50 border rounded-xl text-slate-800 focus:outline-none focus:ring-2 transition-all ${
                    errors.room ? 'border-red-300' : 'border-slate-200 focus:border-clinical-500 focus:ring-clinical-100'
                  }`}
                />
              </div>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-clinical-700 uppercase tracking-wider">Braden Scale / 브레이든 척도</h3>
          ...
          {/* Ulcer Status */}
          <div>
            <h3 className="text-sm font-semibold text-clinical-700 uppercase tracking-wider mb-3">Ulcer Status / 욕창 상태</h3>
